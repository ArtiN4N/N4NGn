...
The Handle_SOA_Map maps a handle to an item. A handle consists of an index and a
...
Example (assumes this package is imported under the alias `hm`):
	...
	entities: hm.Handle_SOA_Map(Entity, Entity_Handle, 1024)
...
Handle :: struct {
	// index into `items` array of the `Handle_SOA_Map` struct.
	idx: u32,
...
Handle_SOA_Map :: struct($T: typeid, $HT: typeid, $N: int) {
...
clear :: proc(m: ^Handle_SOA_Map($T, $HT, $N)) {
...
add :: proc(m: ^Handle_SOA_Map($T, $HT, $N), v: T) -> (HT, bool) #optional_ok {
	v := v
...
get :: proc(m: ^Handle_SOA_Map($T, $HT, $N), h: HT) -> ^T {
...
remove :: proc(m: ^Handle_SOA_Map($T, $HT, $N), h: HT) {
	if h.idx <= 0 || h.idx >= m.num_items {
		return
	}
...
//
// Usage:
//     my_iter := hm.make_iter(&my_Handle_SOA_Map)
...
iter :: proc(it: ^Handle_SOA_Map_Iterator($T, $HT, $N)) -> (val: ^T, h: HT, cond: bool) {
